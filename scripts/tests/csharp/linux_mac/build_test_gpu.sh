...
cp -f ./lib/$ARCH/libsalmon.so ../SalmonLibs.VS2022/Salmon.Test/bin/DebugGPU/net8.0-windows/SalmonNative.dll
elif [[ $OS == "darwin"* ]]; then
make PLATFORM=macos ARCH=$ARCH ENABLE_GPU=1 ENABLE_JNI=1 package
cp -f ./lib/$ARCH/libsalmon.dylib ../SalmonLibs.VS2022/Salmon.Test/bin/DebugGPU/net8.0-windows/SalmonNative.dll
fi
